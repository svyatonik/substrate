	...
	N: crate::changes_trie::BlockNumber,
{
	...
	) -> Result<(Vec<u8>, B::Transaction, Option<ChangesTrieTransaction<H, N>>), Box<dyn Error>> {
	...
	fn execute_aux<R, NC>(
		...
		use_native: bool,
		native_call: Option<NC>,
	) -> (CallResult<R, Exec::Error>, bool, Option<B::Transaction>, Option<ChangesTrieTransaction<H, N>>) where
	...
	) -> (CallResult<R, Exec::Error>, Option<B::Transaction>, Option<ChangesTrieTransaction<H, N>>) where
		R: Decode + Encode + PartialEq,
		NC: FnOnce() -> result::Result<R, &'static str> + UnwindSafe,
		Handler: FnOnce(
	...
	pub fn execute_using_consensus_failure_handler<Handler, R, NC>(
		...
		compute_tx: bool,
		mut native_call: Option<NC>,
	) -> Result<(NativeOrEncoded<R>, Option<B::Transaction>, Option<ChangesTrieTransaction<H, N>>), Box<dyn Error>> where
		R: Decode + Encode + PartialEq,
		NC: FnOnce() -> result::Result<R, &'static str> + UnwindSafe,
		Handler: FnOnce(
			CallResult<R, Exec::Error>,
